...
var utils = require('./utils')
  ...
  , ChunkBuilder = require('./chunkBuilder').ChunkBuilder;
...
/**
 * @description Removes the specified substring.
 * @param {String} source The string to modify.
 * @param {String} value The substring to remove.
 * @param {Boolean} all true to remove all occurences. This parameter is optional.
 * @return {String} A new modified string.
 */
function remove_substr(source, value, all){
	do{
		var index = source.indexOf(value);
		if(index >= 0) source = source.replace(value, '');
		else break;
	} while(all);
	return source;
}
...
function ListPartsResult(rawxml){
	function PartInfo(rawxml){
		...
		if(typeof rawxml == 'string'){
			...
			this.etag = remove_substr(get_element_content(rawxml, 'ETag'), '&quot;', true);
...
Client.prototype.getUploadInfo = function(filename, uploadId, headers, fn){
	this.getAllParts(filename, uploadId, headers, function(err, parts){
		if(err) fn(err, null);
		else{
			var totalSize = 0;
...
Client.prototype.putStream = function (stream, filename, headers, fn) {
    ...
    var req = self.put(filename, utils.merge({'Content-Type': 'application/octet-stream'}, headers));
    req.on('response', function (res) { fn(null, res); });
    stream.on('error', function (err) { fn(err); });
